...
/**
 * Gets the Stripe API version from config (global.stripe_api_version).
 * Used for Stripe client and raw API requests (e.g. beta endpoints).
 * @returns {string|null} API version string or null if not set
 */
function getStripeApiVersion() {
  const config = loadCommandRequirements();
  let version = config.global && config.global.stripe_api_version;
  if (typeof version !== 'string' || !version.trim()) return null;
  version = version.trim();
  // Normalize: header value only (no "Stripe-Version: " prefix)
  if (version.toLowerCase().startsWith('stripe-version:')) {
    version = version.slice(14).trim();
  }
  return version || null;
}
...
/**
 * Returns true if platform name indicates test/UAT environment (-uat or -test suffix)
 * @param {string} platformName - The platform name
 * @returns {boolean}
 */
function isUatOrTestPlatform(platformName) {
  const name = String(platformName || '').toLowerCase();
  return name.endsWith('-uat') || name.endsWith('-test');
}

/**
 * Gets the test connected account ID for a given platform
 * Platforms with -uat or -test suffix use connected_account as test account
 * @param {string} platformName - The platform name
...
function getPlatformTestConnectedAccount(platformName) {
  ...
  if (isUatOrTestPlatform(platformName) && platformConfig.connected_account) {
 ...
 * Platforms without -uat/-test suffix use connected_account as prod account
 * @param {string} platformName - The platform name
...
/**
 * Gets pipeline (Redshift) configuration from config.yml commands.pipeline
 * @returns {Object|null} Pipeline config { host, port, database } or null
 */
function getPipelineConfig() {
  const config = loadCommandRequirements();
  return config.commands && config.commands.pipeline ? config.commands.pipeline : null;
}
