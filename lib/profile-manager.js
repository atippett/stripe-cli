...
/**
 * Check that a file is readable only by the owner (no group/other read).
 * On Windows, permissions are ACL-based so we skip the check.
 * @param {string} filePath - Absolute path to the file
 * @throws {Error} If group or others have read permission on Unix
 */
function assertSecretsFileMode(filePath) {
  ...
  try {
    ...
    if (!stat) return;
    // Unix: mode & 0o44 = group read (0o40) + other read (0o04); must be 0
    const othersCanRead = (stat.mode & 0o44) !== 0;
    if (othersCanRead) {
      throw new Error(
        `.secrets has overly permissive permissions (others can read). ` +
        `Run: chmod 600 ${filePath}`
      );
    }
  } catch (err) {
    if (err.code === 'ENOENT' || err.message.includes('chmod')) throw err;
    // Ignore stat errors (e.g. symlinks) and continue
  }
}
...
class ProfileManager {
  ...
  loadProfiles() {
    ...
    assertSecretsFileMode(this.profilePath);
      ...
      if (inGlobalSection) {
        ...
        if (key.startsWith('test_')) {
          ...
          if (key === 'key') {
            ...
            this.profiles[currentProfile].user = value;
          } else if (key === 'password') {
            this.profiles[currentProfile].password = value;
          }
        }
      }
    }
  ...
  getProfileKey(profileName) {
    // Prefer restricted_key, then key, then secret_key
    const profile = this.getProfile(profileName);
    return profile.restricted_key || profile.key || profile.secret_key || null;
  }

  /**
   * Get the appropriate API key for a specific profile and key type.
   *
   * Restricted-key requests fall back to the profile's secret key when no
   * restricted key is configured (per repo convention: default to restricted,
   * fall back to secret only when restricted is missing). Secret-key requests
   * never fall back to a restricted key.
   *
   * @param {string} profileName - Name of the profile
  ...
  getProfileKeyByType(profileName, keyType, environment = 'prod') {
    ...
    if (keyType === 'restricted') {
      if (environment === 'test') {
        return (
          profile.test_restricted_key ||
          profile.restricted_key ||
          profile.key ||
          profile.test_secret_key ||
          profile.secret_key ||
          null
        );
      }
