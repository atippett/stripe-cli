...
class ProfileManager {
  constructor() {
    this.profilePath = path.join(process.cwd(), '.secrets');
    this.profiles = {};
  ...
  getProfileKeyByType(profileName, keyType, environment = 'prod') {
    ...
    if (keyType === 'secret') {
      ...
      if (environment === 'test') {
        ...
        return profile.restricted_key || profile.key || null; // fallback to old 'key' for compatibility
      }
    }
  ...
  extractPlatformFromProfileName(profileName) {
    ...
    // Split by hyphen and take the first part as platform name
    const parts = profileName.split('-');
    const platformName = parts[0];
    
    // Check if this platform exists in config
    try {
