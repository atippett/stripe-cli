...
function enrichBuyratesRowsWithTargets(rows, fields, buyratesFeeMap, pipelineConfig) {
  ...
  const newRows = visibleRows.map((row) => {
    ...
    /** Match table formatting (4 dp, half-up); same value used for median-vs-target % so pct aligns with buyrate fx. */
    if (targetUsd != null && !Number.isNaN(Number(targetUsd))) {
      targetUsd = roundBuyratesMoney(targetUsd, 4);
    }
    ...
    const formatCell = (val, header, forceDecimals = false, row = null) => {
      ...
      if (!Number.isNaN(num)) {
        ...
        if (
          ...
          // Per-unit pricing columns: show 4 dp (some buy rates carry a 4th decimal).
