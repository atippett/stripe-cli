...
async function mapCardConnect(options) {
  ...
  requireHeaders(ccRows, ['profileid', 'acctid', 'token', 'expiry'], 'CardConnect file');
  requireHeaders(stripeRows, ['old_id', 'created_customer', 'source_new_id'], 'Stripe file');

  const badAcctIds = [];
  for (let i = 0; i < ccRows.length; i++) {
    const val = String(ccRows[i].acctid ?? '').trim();
    if (!/^\d+$/.test(val)) {
  ...
  if (badAcctIds.length) {
    const sample = badAcctIds
      .map(b => `  row ${b.row}: ${b.value === '' ? '(empty)' : JSON.stringify(b.value)}`)
      .join('\n');
    throw new Error(
