...
/**
 * Trim all keys and string values in a row
 * @param {Object} row - Row from CSV parser
 * @returns {Object} - Row with trimmed keys and values
 */
function trimRow(row) {
  const out = {};
  for (const [k, v] of Object.entries(row || {})) {
    const key = String(k).trim();
    const val = v != null && typeof v === 'string' ? v.trim() : v;
    out[key] = val;
  }
  return out;
}
...
/**
 * Format row values for validation error output (masked, no full card numbers).
 * @param {Object} row - Row data from CSV
 * @returns {string} - Debug string showing keys and sample values
 */
function formatRowDebugInfo(row) {
  const keys = Object.keys(row || {});
  const cardVal = row?.card;
  const expVal = row?.exp;
  const safeCard = cardVal == null || cardVal === ''
    ? '(empty)'
    : /^\d{13,19}$/.test(String(cardVal).replace(/\s|-/g, ''))
      ? '***' + String(cardVal).replace(/\s|-/g, '').slice(-4)
      : JSON.stringify(String(cardVal).slice(0, 30));
  const safeExp = expVal == null || expVal === ''
    ? '(empty)'
    : JSON.stringify(String(expVal).slice(0, 10));
  return `card=${safeCard} exp=${safeExp} | keys: [${keys.join(', ')}]`;
}
...
async function createStripeCustomerAndSaveCard(cardData, stripe, accountId, metadata = {}) {
  ...
  // Create payment method - always use raw card data (card number, exp). Do not pass
  // third-party tokens (e.g. CardPointe); Stripe requires raw card details or its own tokens.
...
async function importCards(options) {
  ...
  if (readFromStdin && process.stdin.isTTY) {
    throw new Error('CSV file is required. Use --file option or redirect input (e.g. account.import.card -p platform < file.csv)');
  }
  ...
  try {
    ...
    const firstRowKeys = Object.keys(cards[0] || {}).map(k => normalizeHeaderKey(k));
    const looksLikeCardPointe = firstRowKeys.includes('card number') && firstRowKeys.includes('expiry');
    const useCardPointe = sourceFormat === 'cardpointe' || (sourceFormat === 'default' && looksLikeCardPointe);

    if (useCardPointe) {
      for (let i = 0; i < cards.length; i++) {
        cards[i] = normalizeCardPointeRow(cards[i]);
      }
      console.error(chalk.blue('Input format: CardPointe (columns normalized)' + (looksLikeCardPointe && sourceFormat !== 'cardpointe' ? ' (auto-detected)' : '')));
    }
    ...
    const concurrencyRaw = parseInt(options.concurrency, 10);
    if (isNaN(concurrencyRaw) || concurrencyRaw < 1 || concurrencyRaw > 20) {
      throw new Error('--concurrency must be a positive integer between 1 and 20');
    }
    const concurrency = concurrencyRaw;

    const cardsToProcess = limit ? cards.slice(0, limit) : cards;
    const originalRowsToProcess = limit ? originalRows.slice(0, limit) : originalRows;

    if (limit) {
      console.error(chalk.bold(`  Found ${cards.length} cards in file, processing first ${limit} (--limit ${limit})`));
    } else {
    ...
    if (errors.length > 0) {
      ...
      errors.forEach(error => {
        ...
        console.error(chalk.gray(`    ${error.debug}`));
      });
      console.error('');
    }
    ...
    // Import valid cards
    const totalToImport = cardsToProcess.length;
    ...
    const results = new Array(cardsToProcess.length);
    const validIndices = [];

    for (let i = 0; i < cardsToProcess.length; i++) {
      const card = cardsToProcess[i];
      const validation = validateCardRow(card, i + 2);
      const originalRow = originalRowsToProcess[i] || {};
    ...
    async function processOne(i) {
      ...
      try {
        if (isVerbose) {
          console.error(chalk.gray(`Processing card ${i + 1}/${cardsToProcess.length}: ${maskCardNumber(card.card)}`));
        } else {
      ...
      processedCount++;
      if (!isVerbose) {
        updateProgress(processedCount, maskCardNumber(card.card) + (results[i].stripe_status === 'success' ? ' ✓' : ' ✗'));
      }
    }

    let idx = 0;
    const workers = Array(Math.min(concurrency, validIndices.length)).fill(null).map(async () => {
    ...
    // Results to stdout (CSV or JSON) - mask card numbers in output
    const cardNumberHeaders = ['card', 'card number'];
    const maskCardInRow = (row) => {
      const out = { ...row };
    ...
    if (options.format === 'json') {
      ...
      const getValueForOutput = (row, header) => {
        const val = row[header];
        if (val != null && cardNumberHeaders.some(h => header.toLowerCase().trim() === h)) {
      ...
      const dataLines = results.map(r =>
        [...originalHeaders.map(h => escapeCsv(getValueForOutput(r, h))), ...stripeColumnKeys.map(k => escapeCsv(r[k] ?? ''))].join(',')
      );
