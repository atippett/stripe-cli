...
/**
 * Make a raw HTTP request to Stripe API
 ...
 */
function makeStripeRequest(secretKey, method, path, data = null, accountId = null) {
...
async function enableNetworkCostPassthrough(options) {
  const secretKey = getStripeKey(options, 'account.setting.network-cost.enable');
  ...
  try {
    ...
    // Use raw HTTP request for network costs API (not available in SDK yet)
    const requestData = {
      enabled: 'true'
    };
    
    if (options.startsAt) {
      requestData.starts_at = options.startsAt;
    }
    
    const scheme = await makeStripeRequest(
      secretKey,
      'POST',
      '/v1/pricing_configs/network_costs/schemes',
      requestData,
      options.account
    );
...
async function disableNetworkCostPassthrough(options) {
  const secretKey = getStripeKey(options, 'account.setting.network-cost.disable');
  const stripe = createStripeClient(secretKey);
...
async function getNetworkCostPassthroughStatus(options) {
  const secretKey = getStripeKey(options, 'account.setting.network-cost.status');
  const stripe = createStripeClient(secretKey);
  ...
  try {
    ...
    // Use raw HTTP request for network costs API (not available in SDK yet)
    const pricingConfig = await makeStripeRequest(
      secretKey,
      'GET',
      '/v1/pricing_configs/network_costs?expand[]=current_scheme&expand[]=next_scheme',
      null,
      options.account
    );
...
async function deleteNetworkCostPassthroughScheme(options) {
  const secretKey = getStripeKey(options, 'account.setting.network-cost.delete-scheme');
  const stripe = createStripeClient(secretKey);
  ...
  const confirm = await promptYesNo(chalk.yellow(`Delete network cost passthrough scheme ${options.schemeId} for account ${options.account}? (y/n) `));
  if (!confirm) {
  ...
  try {
    ...
    // Use raw HTTP request for network costs API (not available in SDK yet)
    const deletedScheme = await makeStripeRequest(
      secretKey,
      'DELETE',
      `/v1/pricing_configs/network_costs/schemes/${options.schemeId}`,
      null,
      options.account
    );
