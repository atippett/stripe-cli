...
const { validateKeyForCommand, getStripeApiVersion } = require('./config-loader');

/**
 * Creates and configures a Stripe client instance
 * @param {string} secretKey - Stripe secret key
 * @param {Object} options - Optional. May include apiVersion (Stripe API version string).
 * @returns {Stripe} Configured Stripe client
 */
function createStripeClient(secretKey, options = {}) {
  if (!secretKey) {
  ...
  const apiVersion = options.apiVersion || getStripeApiVersion() || '2023-10-16';
  ...
  if (options.platform) {
    try {
      ...
      if (commandPath) {
        ...
        if (!key) {
          const envPrefix = environment === 'test' ? 'test_' : '';
          const fallbackNote = requiredKeyType === 'restricted'
            ? ` (also tried ${envPrefix}secret_key as fallback)`
            : '';
          throw new Error(
            `Profile '${platform}' has no ${requiredKeyType} key configured for ${environment} environment${fallbackNote}. ` +
            `Command '${commandPath}' requires a ${requiredKeyType} key. ` +
            `Add ${envPrefix}${requiredKeyType}_key to the [${platform}] section in .secrets, or use --key.`
          );
        }
      } else {
        try {
